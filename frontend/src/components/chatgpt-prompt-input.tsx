...
export const PromptBox = React.forwardRef<HTMLTextAreaElement, PromptBoxProps>(
  ({ className, onSend, actionLabel, isSending = false, canStop = false, isCanceling = false, onStop, ...props }, ref) => {
        ...
        <div className="mt-2 flex items-center justify-end">
          {canStop && !hasValue ? (
            <button
              ...
              onClick={onStop}
              className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white transition-colors hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-45"
              aria-label="Pause"
            >
