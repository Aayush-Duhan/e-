...
export function ChatPanel({
  ...
  const isSessionFinished = runId !== null && status === "canceled";
    ...
    : isCanceling
      ...
      : isSessionFinished
        ...
        : status === "awaiting_input"
          ? "The agent is waiting for your input. Type your message below."
          : status === "failed"
          ? "Execution failed. Ask the agent to review or fix the issue."
  ...
  const syncProjectFiles = React.useCallback(async (activeProjectId: string) => {
    try {
      const [sourceRes, outputRes, artifactsRes] = await Promise.all([
        fetch(`/api/projects/${activeProjectId}/files?path=source`, { cache: "no-store" }),
        fetch(`/api/projects/${activeProjectId}/files?path=snowflake`, { cache: "no-store" }),
        fetch(`/api/projects/${activeProjectId}/files?path=artifacts`, { cache: "no-store" }),
      ]);
      ...
      if (artifactsRes.ok) {
        appendFiles(await artifactsRes.json());
      }
  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-hidden">
      <SidebarInset className="relative flex min-h-0 flex-1 flex-col overflow-hidden bg-[#1a1a1a]">
        {/* Wizard background layers — only visible when no active run */}
        {!runId && !isHydratingRun && (
          <>
      ...
      <Workbench chatStarted={!!runId} isStreaming={isAgentThinking} runId={runId ?? undefined} />
    </div>
  );
}
...
function ChatMessageArea({
  ...
  const hasContent = messages.length > 0 || isAgentThinking || streamingContent || error;
          ...
          <div ref={messageRef} className="space-y-3">
            ...
            {streamingContent && streamingContent.content && (
              <div className="flex w-full justify-start">
                <div className="w-full max-w-[min(46rem,92%)] rounded-[calc(0.75rem-1px)] bg-[#232326] px-4 py-1.5 text-white/92">
                  <AssistantMessage content={streamingContent.content} />
                </div>
              </div>
            )}
