...
async function persistTokenToServer(token: string, org: string) {
  try {
...
async function clearTokenFromServer() {
  try {
...
async function fetchBranchesForRepository(repositoryName: string) {
  ...
  if (!state.token.trim() || !org || !trimmedRepository) return;
...
export async function connectRepository() {
  ...
  await persistTokenToServer(token, org);
...
export async function fetchTree() {
  const repositoryName = state.selectedRepositoryName.trim();
  const org = state.org.trim();
  if (!state.token.trim() || !org || !repositoryName) {
    state = {
      ...state,
...
export async function fetchSelectedFiles(): Promise<FetchFilesResult> {
  const repositoryName = state.selectedRepositoryName.trim();
  const org = state.org.trim();
  if (!state.token.trim() || !org || !repositoryName || state.selectedPaths.size === 0) {
    return { files: [] };
  }
